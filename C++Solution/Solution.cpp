/*
 * 2026 Huawei Software Elite Challenge - Preliminary Round
 * Minimum Translation Vector for polygon overlap elimination
 *
 * I/O format (same as demo):
 *   stdin:  n1 n2          (vertex counts, one line)
 *           n1 lines "x y" (polygon A vertices, CCW)
 *           n2 lines "x y" (polygon B vertices, CCW)
 *           "OK"
 *           m              (test case count)
 *           m lines "x y"  (move vectors)
 *           "OK"
 *   stdout: "OK"           (after preprocessing)
 *           m lines "x y"  (MTV results, NO count line)
 *           "OK"
 */
#include <cstdio>
#include <cmath>
...
struct NFPData {
    ...
    // precomputed unique direction vectors (as pairs for speed)
    vector<pair<double,double>> baseDirs;
    bool bothConvex;
    Poly nfp;        // single NFP if both convex
    V refPt;         // B[0]
    BBox globalBB;   // bounding box of entire NFP union
};
...
void computeNFPParts(const Poly& A,const Poly& B,NFPData& data){
    int nb=B.size();
    Poly negB(nb);
    for(int i=0;i<nb;i++)negB[i]=V(-B[i].x,-B[i].y);
    ensureCCW(negB);
    auto tA=triangulate(A);
    auto tNB=triangulate(negB);
    for(auto& ta:tA){Poly t=ta;ensureCCW(t);
        for(auto& tb:tNB){Poly t2=tb;ensureCCW(t2);
...
void precomputeDirs(NFPData& data){
    // use sorted vector for dedup (faster than set for bulk insert)
...
V solveGeneral(V query,const NFPData& data){
    ...
    // candidate directions: base + query-to-vertex of containing parts
    // use sorted array dedup
    struct DK{long long a,b;bool operator<(const DK&o)const{return a!=o.a?a<o.a:b<o.b;}};
    ...
    for(int oi=0;oi<nd;oi++){
        ...
        // for each containing part find ray exit
        ...
        // early termination: if maxExitT >= bestT, skip
        if(maxExitT>=bestT)continue;

        // verify exit point is outside ALL parts (bbox pre-filter)
        double mx=qx+dx*(maxExitT+EPS*100),my=qy+dy*(maxExitT+EPS*100);
        bool allOut=true;
        for(int i=0;i<np;i++){
        ...
        if(allOut){
            ...
            for(int iter=0;iter<20;iter++){
                double mx2=qx+dx*(tc+EPS*100),my2=qy+dy*(tc+EPS*100);
                ...
                for(int idx2:nc){
                    ...
                    double minT=DBL_MAX;
                    for(int i=0;i<pn;i++){int j=(i+1)%pn;
                        double t=raySegT(qx,qy,dx,dy,pv[i].x,pv[i].y,pv[j].x,pv[j].y);
...
// ============================================================
// Main I/O - exact same protocol as Python demo
// ============================================================
int main(){
    int n1,n2;
    scanf("%d%d",&n1,&n2);
    ...
    if(data.bothConvex){
        Poly negB(n2);
        for(int i=0;i<n2;i++)negB[i]=V(-polyB[i].x,-polyB[i].y);
        ensureCCW(negB);
